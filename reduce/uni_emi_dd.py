...
from mutation.utils import insert_list, get_max_node_idx, get_max_edge_idx
...
def construct_deltas(model_dir, edge_dir, test_end):
    ...
    for model_id in range(1, test_end + 1):
        ...
        dead_edges, subs_new_edge, subs_ori_edge = read_mut_info(
            os.path.join(edge_dir, "%d.txt" % model_id)
        )
...
class GraphApplier:
    def __init__(self, model_dir, edge_dir, test_end):
        self.seed_model, self.delta_list, self.name_info_mapping = \
            construct_deltas(model_dir, edge_dir, test_end)
